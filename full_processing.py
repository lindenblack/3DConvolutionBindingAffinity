...
def agrd_to_array(entry_path,dataset_name_num):
    ...
    for line in npdata:
        ...
        # y -= xyz_offset[1]
        # z -= xyz_offset[2]
        MIF[int(x/4),int(y/4),int(z/4)] = target

    # number = entry_path[:-5].split("-")[-1]
    field = entry_path.split("_")[1]
    # new_path =str(number+"_"+field+'.npy')    
    # np.save(f"C:/Users/Linden/GitHub\thing2/3DQSAR/data/EllenMIF/{dataset_name_num}/{new_path}", MIF)
    return MIF,field

# def process_dataset(xyz_offset,dataset_name_num,MIF_nodes):
def process_dataset(dataset_name_num):
    parent_dir = r"C:\Users\Linden\GitHub\thing2\3DQSAR\data\EllenMIF\1_10"
    ...
    for entry in os.scandir(open3D_directory):
        if (entry.path.endswith(".agrd")):
            ...
            MIF, field = agrd_to_array(name,dataset_name_num)    
            if field == "fld-01":
                X_1.append(MIF)
            elif field == "fld-02":
                X_2.append(MIF)

    path = os.path.join(r"C:/Users/Linden/GitHub/thing2/3DQSAR/data/Ellen2Step/",dataset_name_num)
    outfile = open(path,"wb")
    pickle.dump([X_1,X_2],outfile)
    outfile.close()


def generate_labels():
    path = os.path.join(r"C:\Users\Linden\GitHub\thing2\3DQSAR\data\DatasetsEdited",dataset_name_num)
...
def func():
    ds_num=0
    for entry in os.scandir(r"C:\Users\Linden\GitHub\thing2\3DQSAR\data\EllenRawData\reduced"):
        if (entry.path.endswith(".sdf")):
            ...
            set_globals()
            process_dataset(dataset_name_num)
